...
// Fix coach names for sessions
const fixSessionCoachNames = async () => {
  try {
    console.log('🔧 Fixing coach names for sessions...');
    
    // Get all sessions
    const sessions = await Session.find();
    console.log(`Found ${sessions.length} sessions`);
    
    // Get unique coach IDs from sessions
    const coachIds = [...new Set(sessions.map(session => session.coach.toString()))];
    console.log(`Unique coach IDs in sessions: ${coachIds.join(', ')}`);
    
    for (const coachId of coachIds) {
      console.log(`\n👤 Processing coach: ${coachId}`);
      ...
      if (!coach) {
        ...
        await coach.save();
        console.log(`  ✅ Created new coach with phone: ${coach.phone}`);
      } else {
        console.log(`  📋 Coach exists with phone: ${coach.phone}`);
      }
    ...
    console.log('\n🎉 Coach names for sessions fixed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing session coach names:', error);
  }
};
