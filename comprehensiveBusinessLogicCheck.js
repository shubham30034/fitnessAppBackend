...
const comprehensiveBusinessLogicCheck = async () => {
  try {
    console.log('🔍 COMPREHENSIVE BUSINESS LOGIC CHECK');
    console.log('=====================================\n');
    ...
    for (const student of students) {
      ...
      if (activeSubscription) {
        ...
        if (studentSessions.length > 0) {
          ...
          console.log(`  ❌ Student ${student.phone}: Expired subscription but ${studentSessions.length} sessions (VIOLATION!)`);
        } else {
          console.log(`  ✅ Student ${student.phone}: Expired subscription, no sessions`);
        }
      } else if (anySubscription) {
        studentsWithExpiredSubs++;
        if (studentSessions.length > 0) {
          studentsWithSessionsButNoSubs++;
          console.log(`  ❌ Student ${student.phone}: Inactive subscription but ${studentSessions.length} sessions (VIOLATION!)`);
        } else {
          console.log(`  ✅ Student ${student.phone}: Inactive subscription, no sessions`);
        }
      } else {
        studentsWithNoSubs++;
        if (studentSessions.length > 0) {
    ...
    console.error('❌ Error in comprehensive business logic check:', error);
  }
};
