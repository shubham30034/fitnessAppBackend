...
// Fix subscription correctly
const fixSubscriptionCorrectly = async () => {
  try {
    console.log('🔧 Fixing subscription correctly for coach 8798675686...');
    ...
    // The original subscription expired on July 26, 2025
    const originalEndDate = new Date('2025-07-26');
    ...
    if (originalEndDate < today) {
      ...
      // Deactivate the subscription with correct enum value
      subscription.isActive = false;
      subscription.expiredAt = new Date();
      subscription.expirationReason = 'automatic_expiration'; // Use correct enum value
    ...
    console.log(`  Expired At: ${finalSubscription.expiredAt ? new Date(finalSubscription.expiredAt).toDateString() : 'N/A'}`);
    console.log(`  Expiration Reason: ${finalSubscription.expirationReason || 'N/A'}`);
...
const main = async () => {
  try {
    await connectDB();
    await fixSubscriptionCorrectly();
    process.exit(0);
  } catch (error) {
